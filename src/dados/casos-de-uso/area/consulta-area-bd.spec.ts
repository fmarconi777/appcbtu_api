...
describe('ConsultaAreaBD', () => {
  ...
  test('Deve retornar null caso um parâmetro inválido seja fornecido', async () => {
    const { sut, repositorioAreaStub } = makeSut()
    jest.spyOn(repositorioAreaStub, 'consultar').mockReturnValueOnce(Promise.resolve(null))
    const area = 'AREA_QUALQUER'
